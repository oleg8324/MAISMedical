'------------------------------------------------------------------------------
' <auto-generated>
'    This code was generated from a template.
'
'    Manual changes to this file may cause unexpected behavior in your application.
'    Manual changes to this file will be overwritten if the code is regenerated.
' </auto-generated>
'------------------------------------------------------------------------------

Imports System
Imports System.Collections.Generic

Partial Public Class Person_Type
    Public Property Person_Type_Sid As Integer
    Public Property Person_Type_Code As String
    Public Property Person_Type_Desc As String
    Public Property Start_Date As Date
    Public Property End_Date As Date
    Public Property Active_Flg As Boolean
    Public Property Create_Date As Date
    Public Property Create_By As Integer
    Public Property Last_Update_Date As Date
    Public Property Last_Update_By As Integer

    Public Overridable Property RN_DD_Person_Type_Xref As ICollection(Of RN_DD_Person_Type_Xref) = New HashSet(Of RN_DD_Person_Type_Xref)

End Class
