...
$(document).ready(function () {
    //if ($(".rblNewRnTypeSelect").val() != 'undefined') {
    //    if ($(".rblNewRnTypeSelect").val() == null) {
    //        $(".dvNewCourseTable").addstyle("display", "Null");
    //    }
    //}
    //
    ...
    var TestCerificationStartDateIsAllowed = function () {
        var RnDataTest = {
            'RNsID': $(".ddlRnNames").val()
        };


        executePageMethod('ManageCourses.aspx', 'GetRnsCertificationMinStartDate', "{'RnData':" + JSON.stringify(RnDataTest) + "}", function (data) {

            if (!(data.d == '1/1/0001')) {

                CertificationStartDate = new Date(data.d);


                //if (EffStartDate < MinDate) {
                //    alert("The date entered is not allowed. Must enter a date on or after " + data.d + ".");
    ...
    var GetCerificationMinStartDate = function () {
        ...
        executePageMethod('ManageCourses.aspx', 'GetRnsCertificationMinStartDate', "{'RnData':" + JSON.stringify(RnDataTest) + "}", function (data) {
            ...
            if (!(data.d == '1/1/0001')) {
                ...
                $('.txtEffectiveStartDate').calendarsPicker('option', 'minDate', data.d);

            } else {
                $('.txtEffectiveStartDate').calendarsPicker('option', 'minDate', '');
